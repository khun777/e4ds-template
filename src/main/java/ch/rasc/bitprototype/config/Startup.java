...
import ch.rasc.bitprototype.util.CommitJDBCAppender;
...
public class Startup implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		if (new JPAQuery(entityManager).from(QUser.user).count() == 0) {
			...
			normalUser.setPasswordHash(passwordEncoder.encode("bedarf"));
			...
			normalUser.setPasswordHash(passwordEncoder.encode("einkauf"));
			normalUser.setEnabled(true);
			...
			normalUser.setPasswordHash(passwordEncoder.encode("lieferant"));
			normalUser.setEnabled(true);
